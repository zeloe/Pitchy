...
private:
    ...
    void updateFftSize (const int newFftSize)
    {
        ...
        prevBuffer.setSize(numChannels, fftSize);
        prevBuffer.clear();
    ...
    virtual void modification()
    {
        ...
        float* prevPhase = prevBuffer.getWritePointer(0);
        for(int i = 0; i < fftSize; i++)
        {
            stochBuffer[i] = std::complex<float>(0, 0); // Initialize the buffer with zero complex values
        }
        

        // Modify the magnitude component for each frequency bin
        for(int i = 0; i < fftSize; i++)
        {
            float tempMag = (frequencyDomainBuffer[i].real()); // Extract the magnitude
            float tempPhas = (frequencyDomainBuffer[i].imag());
            int new_index =  static_cast<int>(i * shift_factor);
            if (i * shift_factor > fftSize)
            {
                break;
            }
                
            new_index = new_index%fftSize;
            const float preFrame = prevPhase[i];
            stochBuffer[new_index].real(tempMag);
            stochBuffer[new_index].imag(tempPhas + preFrame);
        }

        for(int i = 0; i < fftSize; i++)
        {
            prevPhase[i] = stochBuffer[i].imag();
        }
        ...
        for (int index = 0; index < fftSize / 2 + 1; ++index) {
            ...
            */
            ...
            fft->perform(stochBuffer.get(), timeoutbufferBuffer.get(), true);
    }
